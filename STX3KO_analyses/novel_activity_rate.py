    ...
    def plot_single_session(self, cond, mouse, day, 
        ...
        if baseline_corr:
            ...
            if log:
                ...
                comb = np.nanmean(np.log(np.concatenate((data['baseline_act'], data['block_act']))/b),axis=-1)
                fam = np.nanmean(np.log(data['block_fam_act']/b), axis=-1)
                nov = np.nanmean(np.log(data['block_nov_act']/b), axis=-1)
            else:
