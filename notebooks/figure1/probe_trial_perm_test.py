...
def run_licks():
    ...
    for (cond, mice) in zip(('ctrl', 'ko'), (ctrl_mice, ko_mice)):
        for day in range(6):
            ...
            for mouse in mice:
                print(mouse)
                sess = stx.utilities.load_vr_day(mouse, day, trial_mat_keys = ('licks_sum','reward', 'speed'),verbose = False, pkldir = '/mnt/BigDisk/YMaze_VR_Pkls/')
                ...
                for ttype in ('fam', 'nov'):
                    ...
                    rzone_licks = np.nanmean(licks[:,rzone-5:rzone+3], axis=0).sum()
                    rzone_speed = np.nanmean(speed[:,rzone-5:rzone+3], axis=0).sum()
    ...
    df['rzone_speed_rank'] = df['rzone_speed'].rank()
...
if __name__ == "__main__":
    ...
    with open('/home/mplitt/shuffle_pkls/probe_trial_licks_sum_mixedlm_shuffle.pkl', 'wb') as file:
